    ...
    <div class="page-title">
        <div class="row">
            <div class="col-12 col-md-6 order-md-1 order-last">
                <h3>Lembaga Pemberdayaan Masyarakat Desa (LPMD)</h3>
                {{-- <p class="text-subtitle text-muted">Data LPMD</p> --}}
            </div>
...
@section('content')
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table" id="table-lpmd">
                        <thead>
                            <tr>
                            <th>No</th>
                                <th>Nama</th>
    ...
    <!-- Basic Tables end -->
@include('admin.pemerintahan.lembaga_desa.lpmd.modal.create')     
@include('admin.pemerintahan.lembaga_desa.lpmd.modal.update')     
@include('admin.pemerintahan.lembaga_desa.lpmd.modal.delete') 
...
<script>
    const table = new DataTable('#table-lpmd');

    const url_index = `{{ route('admin.api.lpmd.index') }}`;
    const url_show = (id) => `{{ route('admin.api.lpmd.show', ':id') }}`.replace(':id', id);
    const url_store = `{{ route('admin.api.lpmd.store') }}`;
    const url_update = (id) => `{{ route('admin.api.lpmd.update', ':id') }}`.replace(':id', id);
    const url_destroy = (id) => `{{ route('admin.api.lpmd.destroy', ':id') }}`.replace(':id', id);

    // button markup

    function rowData(lpmd) {
        return [
            lpmd.id,
            lpmd.Nama,
            lpmd.Jabatan,
            lpmd.Alamat,
            `
            <button type="button" class="btn btn-outline-primary" data-bs-toggle="modal" data-bs-idlpmd="${lpmd.id}" data-bs-target="#editModal">
                Edit
            </button>
            <button type="button" class="btn btn-primary block" data-bs-toggle="modal"
                data-bs-target="#deleteModal" data-bs-idlpmd="${lpmd.id}">
                Hapus
            </button>
            `
        ];
...
<script>
    ...
    updateButton.addEventListener('click', function (event) {
        const id = updateButton.getAttribute('data-bs-idlpmd');
...
<script>
    ...
    deleteModal.addEventListener('show.bs.modal', function (event) {
        const button = event.relatedTarget
        const idlpmd = button.getAttribute('data-bs-idlpmd');

        destroyButton.setAttribute('data-bs-idlpmd', idlpmd);
        console.log('del:', idlpmd);
    });

    destroyButton.addEventListener('click', function (event) {
        const idlpmd = this.getAttribute('data-bs-idlpmd');
        const url = url_destroy(idlpmd);
        ...
        $.ajax(url, {
            ...
            success: function (response) {
                table.row($(`button[data-bs-idlpmd="${idlpmd}"]`).parents('tr')).remove().draw(false);
                showToast('success', response.message);
            },
            error: function (error) {
                console.error('Error:', error);
