...
@section('content')
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table" id="table-linmas">
                        <thead>
                            <tr>
                            <th>No</th>
                                <th>Nama</th>
    ...
    <!-- Basic Tables end -->
@include('admin.pemerintahan.lembaga_desa.linmas.modal.create')     
...
<script>
    const table = new DataTable('#table-linmas');

    const url_index = `{{ route('admin.api.linmas.index') }}`;
    const url_show = (id) => `{{ route('admin.api.linmas.show', ':id') }}`.replace(':id', id);
    const url_store = `{{ route('admin.api.linmas.store') }}`;
    const url_update = (id) => `{{ route('admin.api.linmas.update', ':id') }}`.replace(':id', id);
    const url_destroy = (id) => `{{ route('admin.api.linmas.destroy', ':id') }}`.replace(':id', id);

    // button markup

    function rowData(linmas) {
        return [
            linmas.id,
            linmas.Nama,
            linmas.Jabatan,
            linmas.Alamat,
            `
            <button type="button" class="btn btn-outline-primary" data-bs-toggle="modal" data-bs-idlinmas="${linmas.id}" data-bs-target="#editModal">
                Edit
            </button>
            <button type="button" class="btn btn-outline-primary block" data-bs-toggle="modal"
                data-bs-target="#deleteModal" data-bs-idlinmas="${linmas.id}">
                Hapus
            </button>
            `
        ];
...
<script>
    ...
    updateButton.addEventListener('click', function (event) {
        const id = updateButton.getAttribute('data-bs-idlinmas');
...
<script>
    ...
    deleteModal.addEventListener('show.bs.modal', function (event) {
        const button = event.relatedTarget
        const idlinmas = button.getAttribute('data-bs-idlinmas');

        destroyButton.setAttribute('data-bs-idlinmas', idlinmas);
        console.log('del:', idlinmas);
    });

    destroyButton.addEventListener('click', function (event) {
        const idlinmas = this.getAttribute('data-bs-idlinmas');
        const url = url_destroy(idlinmas);
        ...
        $.ajax(url, {
            ...
            success: function (response) {
                table.row($(`button[data-bs-idlinmas="${idlinmas}"]`).parents('tr')).remove().draw(false);
                showToast('success', response.message);
            },
            error: function (error) {
                console.error('Error:', error);
