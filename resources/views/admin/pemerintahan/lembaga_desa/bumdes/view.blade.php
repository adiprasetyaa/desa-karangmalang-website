    ...
    <div class="page-title">
        <div class="row">
            <div class="col-12 col-md-6 order-md-1 order-last">
                <h3>Badan Usaha Milik Desa (BUMDES)</h3>
                {{-- <p class="text-subtitle text-muted">Data BUMDES</p> --}}
            </div>
...
@section('content')
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table" id="table-bumdes">
                        <thead>
                            <tr>
                                <th>No</th>
                                <th>Nama</th>
    ...
    <!-- Basic Tables end -->
@include('admin.pemerintahan.lembaga_desa.bumdes.modal.create')     
@include('admin.pemerintahan.lembaga_desa.bumdes.modal.update')     
@include('admin.pemerintahan.lembaga_desa.bumdes.modal.delete') 
...
<script>
    const table = new DataTable('#table-bumdes');

    const url_index = `{{ route('admin.api.bumdes.index') }}`;
    const url_show = (id) => `{{ route('admin.api.bumdes.show', ':id') }}`.replace(':id', id);
    const url_store = `{{ route('admin.api.bumdes.store') }}`;
    const url_update = (id) => `{{ route('admin.api.bumdes.update', ':id') }}`.replace(':id', id);
    const url_destroy = (id) => `{{ route('admin.api.bumdes.destroy', ':id') }}`.replace(':id', id);

    // button markup

    function rowData(bumdes) {
        return [
            bumdes.id,
            bumdes.Nama,
            bumdes.Jabatan,
...
<script>
    ...
    updateButton.addEventListener('click', function (event) {
        const id = updateButton.getAttribute('data-bs-idbumdes');
...
<script>
    ...
    deleteModal.addEventListener('show.bs.modal', function (event) {
        const button = event.relatedTarget
        const idbumdes = button.getAttribute('data-bs-idbumdes');

        destroyButton.setAttribute('data-bs-idbumdes', idbumdes);
        console.log('del:', idbumdes);
    });

    destroyButton.addEventListener('click', function (event) {
        const idbumdes = this.getAttribute('data-bs-idbumdes');
        const url = url_destroy(idbumdes);
        ...
        $.ajax(url, {
            ...
            success: function (response) {
                table.row($(`button[data-bs-idbumdes="${idbumdes}"]`).parents('tr')).remove().draw(false);
                showToast('success', response.message);
            },
            error: function (error) {
                console.error('Error:', error);
