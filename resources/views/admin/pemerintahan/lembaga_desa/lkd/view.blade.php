    ...
    <div class="page-title">
        <div class="row">
            <div class="col-12 col-md-6 order-md-1 order-last">
                <h3>Lembaga Kemasyarakatan Desa (LKD)</h3>
                {{-- <p class="text-subtitle text-muted">Data LKD</p> --}}
            </div>
...
@section('content')
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table" id="table-lkd">
                        <thead>
                            <tr>
                                <th>No</th>
                                <th>Nama</th>
    ...
    <!-- Basic Tables end -->
@include('admin.pemerintahan.lembaga_desa.lkd.modal.create')     
@include('admin.pemerintahan.lembaga_desa.lkd.modal.update')     
@include('admin.pemerintahan.lembaga_desa.lkd.modal.delete') 
...
<script>
    const table = new DataTable('#table-lkd');

    const url_index = `{{ route('admin.api.lkd.index') }}`;
    const url_show = (id) => `{{ route('admin.api.lkd.show', ':id') }}`.replace(':id', id);
    const url_store = `{{ route('admin.api.lkd.store') }}`;
    const url_update = (id) => `{{ route('admin.api.lkd.update', ':id') }}`.replace(':id', id);
    const url_destroy = (id) => `{{ route('admin.api.lkd.destroy', ':id') }}`.replace(':id', id);

    // button markup

    function rowData(lkd) {
        return [
            lkd.id,
            lkd.Nama,
            lkd.Jabatan,
            `
            <button type="button" class="btn btn-outline-primary" data-bs-toggle="modal" data-bs-idlkd="${lkd.id}" data-bs-target="#editModal">
                Edit
            </button>
            <button type="button" class="btn btn-outline-primary block" data-bs-toggle="modal"
                data-bs-target="#deleteModal" data-bs-idlkd="${lkd.id}">
                Hapus
            </button>
            `
        ];
...
<script>
    ...
    updateButton.addEventListener('click', function (event) {
        const id = updateButton.getAttribute('data-bs-idlkd');
...
<script>
    ...
    deleteModal.addEventListener('show.bs.modal', function (event) {
        const button = event.relatedTarget
        const idlkd = button.getAttribute('data-bs-idlkd');

        destroyButton.setAttribute('data-bs-idlkd', idlkd);
        console.log('del:', idlkd);
    });

    destroyButton.addEventListener('click', function (event) {
        const idlkd = this.getAttribute('data-bs-idlkd');
        const url = url_destroy(idlkd);
        ...
        $.ajax(url, {
            ...
            success: function (response) {
                table.row($(`button[data-bs-idlkd="${idlkd}"]`).parents('tr')).remove().draw(false);
                showToast('success', response.message);
            },
            error: function (error) {
                console.error('Error:', error);
